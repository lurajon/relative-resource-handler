...
public class HelloWorldController
{
    ...
    public String getChangeClientUrl()
    {
        FacesContext facesContext = FacesContext.getCurrentInstance();
        String requestUrl = ((HttpServletRequest) facesContext.getExternalContext().getRequest()).getRequestURL().toString();

        // change 127.0.0.1 to localhost and vice versa
        if (requestUrl.contains("127.0.0.1"))
        {
            requestUrl = requestUrl.replace("127.0.0.1", "localhost");
        }
        else
        {
